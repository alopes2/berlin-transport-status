...
if ! grep -Eq 'default_route_settings[[:space:]]*\{' "$api_gateway"; then
  echo "API Gateway must define default route throttling." >&2
  exit 1
fi

if ! grep -Eq 'throttling_rate_limit[[:space:]]*=[[:space:]]*var\.api_throttling_rate_limit' "$api_gateway"; then
  echo "API Gateway must configure a sustained request rate limit." >&2
  exit 1
fi

if ! grep -Eq 'throttling_burst_limit[[:space:]]*=[[:space:]]*var\.api_throttling_burst_limit' "$api_gateway"; then
  echo "API Gateway must configure a burst request limit." >&2
  exit 1
fi

if ! grep -Fq 'variable "api_throttling_rate_limit"' "$variables"; then
  echo "Terraform must expose the API throttling rate limit." >&2
  exit 1
fi

if ! grep -Fq 'variable "api_throttling_burst_limit"' "$variables"; then
